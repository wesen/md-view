...
- [x] 9.2 Rewrite **`README.md`**: drop "lightweight daemon"/browser/`serve`/`status`/`stop`; replace install (build-from-source via `make build` + GoReleaser-produced native packages — NOT `go install .../cmd/md-view`, that path is deleted and CGO/Wails won't `go install`); new architecture diagram (single Wails process: Cobra + Wails runtime + WebView + `pkg/renderer`/`pkg/watcher`); correct command table (`md-view view [file] [--dark]`, bare `md-view`); features (native window, live reload via events, `/file/` image serving, Mermaid, dual-theme, frontmatter, copy/reMarkable/download, drag-drop, recent files); note Linux `libwebkit2gtk-4.1-dev` + `libsoup-3.0-dev` requirement and the known Linux multi-window limitation.
- [x] 9.3 Rewrite **`docs/getting-started.md`**: install (build from source + native packages), first view (native window, not browser tab), live reload (event-driven — drop `--no-reload`), dark theme (toggle + `--dark` — drop URL param/localStorage), Mermaid, multiple files (each `view` opens a window — note multi-window behavior), i3/Sway (native window titles `md-view: <filename>`). Remove `serve`/`status`/`stop`/browser-selection/URL-param sections.
- [x] 9.4 Rewrite **`docs/user-guide.md`** (major surgery): REMOVED `serve`/`status`/`stop` command sections, HTTP API (render/raw/static/SSE), Unix Socket Protocol, Daemon Management (state files, stale PID), Browser Integration (browser selection), and `--browser`/`--no-browser`/`--no-reload`/`--port` flags. KEPT + updated Markdown Features, Syntax Highlighting ("server-side" → "in-process"), Mermaid Diagrams, YAML Frontmatter, Page Titles → Window Titles (native window). REWROTE the `view` command (only `--dark`), Dark Theme (in-memory), Live Reload (Wails events), i3/Sway (native window) + Multiple windows, Security (`/file/` allow-list, no socket/HTTP), Troubleshooting (webkit dev libs, build-tags error, multi-window, CGO), Dependencies (Wails/cobra/goldmark/chroma/fsnotify/logcopter; dropped glazed).
- [x] 9.5 Verified **`AGENT.md`** has no stale operational references (its "daemon"/"socket" mentions at line 32 are historical context explaining the cutover — kept). No rewrite needed.
- [x] 9.6 Updated ticket bookkeeping: diary Steps 13-15, `index.md` summary/status, `changelog.md` entry, file relations (diary + review doc ↔ rewritten docs).
- [x] 9.7 Final validation: `grep -rnE 'md-view serve|md-view stop|md-view status|--browser|--no-reload|--no-browser|Unix Socket|SSE|/render\?|/events\?|127\.0\.0\.1' README.md AGENT.md docs/` → zero matches. Surviving `daemon`/`go install`/`cmd/md-view` hits are all intentional negatives/migration notes.
